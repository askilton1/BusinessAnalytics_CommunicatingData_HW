...
```{r,message=FALSE,results='asis',fig.align='center',echo=FALSE,fig.width=5}
...
ggplot(filter(AdData.melt,variable!="AdSpending"),aes(x=Market,y=value,color=variable,alpha=0.75)) +
  ...
  theme(legend.position="bottom",legend.title=element_blank(),
        ...
        plot.title=element_text(family="Times"),legend.text=element_text(family="Times"),
        panel.background = element_rect(fill = "white")) + 
  ggtitle("Figure 1: Distribution of Advertising Spending \nby Market and Advertising Medium") + 
  ylab("Advertising Spending (in $1K)") + 
  scale_alpha(guide="none")
AdData <- select(AdData,-row_number)
ggsave("box_and_whisker.png",width=5,height=5,units="in")
```
...
```{r, message=FALSE, warning=FALSE,fig.align='center',fig.height=3,echo=FALSE,fig.width=5}
library(ggplot2)
...
grid.arrange(plot_Sales,plot_Adspending,ncol=2,top=textGrob("Figure 2: Histograms of Sales and \nTotal Advertising Spending", gp=gpar(fontsize=15,fontfamily="Times")))
```

The same correlation is perhaps better visualized using a dotplot:
```{r,fig.height=4,echo=FALSE,fig.align='center',fig.width=5}
ggplot(AdData,aes(x=AdSpending,y=Sales,color=Market,alpha=0.75)) + 
  ...
  theme(legend.position="bottom",legend.title=element_blank(),
        axis.title=element_text(family="Times"),axis.text=element_text(family="Times"),
        legend.text=element_text(family="Times"),plot.title=element_text(family="Times"),
        panel.background = element_rect(fill = "white")) +
  ggtitle("Figure 3: Sales by Advertising Spending")
```
...
ggplot(filter(AdData.melt,variable!="AdSpending"),aes(x=value,y=Sales/1000,alpha=0.75)) + 
  ...
  theme(legend.position="none",
        axis.title=element_text(family="Times"),axis.text=element_text(family="Times"),
        plot.title=element_text(family="Times"),panel.background = element_rect(fill = "white")) + 
  geom_smooth(color="black") + 
  facet_grid(Market~variable,scales="free") + 
  ggtitle("Figure 4: Sales by Advertising Spending") + 
  ylab("Sales (in $1M)") + 
  xlab("Advertising Spending (in $1K)")
